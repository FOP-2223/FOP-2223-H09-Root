...
public class SequenceOfAssertions {
    ...
    /**
     * Checks the basics of the "of" method of a Sequence class.
     */
    public static Method checkBasics(
        ...
        final Method ofMethod = Assertions.assertDoesNotThrow(() ->
            ...
            "Class " + enclosingClass.getSimpleName() + " does not have method of with parameter of type "
        ...
        if (!(ofMethod.getGenericReturnType() instanceof ParameterizedType parameterizedFunction)) {
            throw new AssertionFailedError("Method " + enclosingClass.getSimpleName()
                + ".of should parameterize return type Function");
        }
        ...
        if (!(functionT instanceof ParameterizedType pFunctionT)) {
            throw new AssertionFailedError("Method " + enclosingClass.getSimpleName()
                + ".of does not parameterize the first type parameter of Function"
                + " (" + functionT.getTypeName() + ")");
        }
        if (!(functionR instanceof ParameterizedType pFunctionR)) {
            throw new AssertionFailedError("Method " + enclosingClass.getSimpleName()
                + ".of does not parameterize the second type parameter of Function"
                + " (" + functionR.getTypeName() + ")");
        }

        Assertions.assertEquals(Sequence.class, pFunctionT.getRawType(),
            "Method " + enclosingClass.getSimpleName()
                + ".of should parameterize the first type parameter of Function with Sequence");
        Assertions.assertEquals(Sequence.class, pFunctionR.getRawType(),
            "Method " + enclosingClass.getSimpleName()
                + ".of should parameterize the second type parameter of Function with Sequence");
