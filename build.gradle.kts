...
tasks {
    ...
    val graderJar by creating(Jar::class) {
        group = "build"
        afterEvaluate {
            archiveFileName.set("FOP-2223-H09-Root-${project.version}.jar")
    ...
    val graderLibs by creating(Jar::class) {
        ...
        val runtimeDeps = grader.runtimeClasspath.mapNotNull {
            if (it.path.toLowerCase().contains("h09") || jagrRuntime.contains(it)) {
                null
        ...
        from(runtimeDeps)
        archiveFileName.set("FOP-2223-H09-Root-${project.version}-libs.jar")
    }
    create("graderAll") {
        group = "build"
